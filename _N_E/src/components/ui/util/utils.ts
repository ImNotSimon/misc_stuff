...
): readonly [Omit<T, K> | T, Pick<T, K> | any] => {
  if (!variantKeys) {
    return [props, {}];
  }
...
>(
  ...
  const props = Object.keys(originalProps)
    .filter(
      (key) =>
        !variantKeys.includes(key as VK) || commonKeys?.includes(key as CK),
    )
    .reduce(
      (acc, key) => ({ ...acc, [key]: originalProps[key as keyof P] }),
      {},
    ) as Omit<P, Exclude<VK, CK>>;
